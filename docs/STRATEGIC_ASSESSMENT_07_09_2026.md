...
- **2026-07-13 — control arm dead on arrival, re-pinned to the local model (owner decision).** The `openrouter/anthropic/claude-sonnet-5` pin failed on 100% of cycles from launch (84/84 per agent over ~7h): OpenRouter HTTP 402, the key has ~$0.30 of credit and opencode requests 32k max output tokens. Owner chose re-pinning over funding. Changes: `llm/model_chain.py` `_tier_from_pinned_value` — the literal pin `"llama_server"` now dispatches to the local llama-server tier (was hardcoded to opencode kind, making a local pin impossible); control arm re-pinned in the live DB and in `scripts/fresh_start.py`; settings configured (binary = Ollama's bundled `llama-server.exe`, model = `Qwen3.6-35B-A3B-UD-Q4_K_S.gguf`, port 8085, `reasoning=false` i.e. thinking OFF per the CLAUDE.md empirics, `spawn_on_startup=true` for future boots); server started inside the running forge process via `POST /api/local-server/start` — no desk restart. Verified: `/health` ok, real pinned decision in 8.0s labelled "Local llama-server (Qwen3.6)". **Analysis note:** the control arm's sample effectively begins 2026-07-13 ~04:30 UTC; its first ~7h are error rows (zero trades, zero P&L impact). Compiled-vs-control comparisons should date the control arm from the re-pin, and the arm now answers "does LLM reasoning (local Qwen, thinking off) beat compiled specs?" — not the original frontier-model question.
