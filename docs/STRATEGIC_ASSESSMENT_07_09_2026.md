...
## 9. Pre-run final review — 2026-07-11 (launch-gate re-audit)

**Context.** The pre-run gate (§8: R1, R2, R4, R5, R3-capture, R10-trimmed, R12) was reported complete and the desk was found *already running* on `feat/r5-cost-model` when this review started. The review audited the branch code, the live `data/forge.db`, and the July ledger against the gate's acceptance criteria before committing to the two-week unattended run.

**Verdict.** The gate was not closed. R1, R12, the R5 wick-reconciliation, and most of R2/R4 were genuinely done — but the shared cost model at the heart of R5 had two arithmetic defects worse than the M1/M2 ones it replaced, the R3-capture and R10 gate items had not landed at all, and the live DB predated the R2 seeding code entirely. Every finding below was fixed the same day (same branch); resolutions name the mechanism so R6–R9 know what changed under them.

### 9.1 Findings and resolutions

| # | Severity | Finding | Resolution (2026-07-11) |
|---|---|---|---|
| F1 | **Blocker** | **Leverage² P&L.** `execution/costs.py compute_gross_pnl` computed `pnl_usd = true_notional × price_move × leverage` — but true_notional already contains leverage, so every leveraged trade booked leverage× the correct dollar P&L (a 3x trade booked 3×). Poisoned balances, drawdowns (the evaluator's 20% suspend fired at ~6.7% real for 3x), and both paper *and* backtest identically. `tests/test_costs.py` **asserted the wrong math as expected** — tests mirrored the implementation, not the exchange. | `pnl_usd = true_notional × price_move`; `pnl_pct` stays `move × leverage` (return on margin, exchange ROE). Tests rewritten to exchange-true values first (watched fail, then fixed); regression test `test_pnl_usd_not_double_leveraged` pins it. Downstream pinned-wrong expectations corrected in `test_positions_ledger`, `test_paper_bridge`, `test_backtest_parity`. |
| F2 | **Blocker** | **Price-less funding.** `compute_funding_pnl` paid `coins × rate` — dimensionally wrong (funding is a fraction of position *value*). BTC funding understated ~64,000×; sub-$1 assets overstated ~10×. Funding harvest — the proposal's #1 paradigm and `silver_basin`'s whole thesis — was unmeasurable in both paper and backtest. | Funding = `true_notional × rate` per event (API renamed `position_size_coins` → `true_notional` in `compute_funding_pnl` / `compute_funding_pnl_simple`; callers in `store/positions.py` and `backtest/engine.py` updated). Exact-arithmetic tests replaced the sign-only ones. |
| F3 | **Blocker** | **Stale roster in the live DB — and the desk was live-trading on it.** `iron_moth`/`silver_basin`/`jade_hawk` had `config_json = {}` (not compiled, no specs rows); the control arm had no `pinned_model`; the day's ledger showed decisions from free-tier roulette models ("Big Pickle", "North Mini Code Free"). The DB predated R2's seeding code; forge.py only seeds an *empty* DB, and its compiled-reconciliation reads the very flag that was missing. A two-week run on this DB answers no question. | Desk stopped (PID killed). `scripts/fresh_start.py --yes` re-seeded: verified 4 compiled agents with active spec rows, 3 control-arm agents pinned to `openrouter/anthropic/claude-sonnet-5`, benchmarks seeded, 11 accounts at $50k. Fresh-start-before-launch is now an explicit step in the §9.3 checklist. |
| F4 | **Blocker** | **R3-capture (gate item 5) never landed.** LLM-agent wait decisions logged `details=None` — no candidate asset/direction/SL/TP — so `store/counterfactuals.py` skipped every wait; coverage would have been ~0% for the whole run, silently (the job "succeeds" while filling nothing). | Decision prompt now requires a `candidate` block on every wait (`agents/prompt_builder.py`); `agents/decision_loop.py` validates and persists it (`_extract_wait_candidate`: strict shape, heartbeat price fills a missing entry, malformed candidates dropped — never guessed). Covered by three decision-loop tests + smoke assertion. |
| F5 | **Blocker** | **R10 smoke harness did not exist.** No `tests/test_smoke.py`, no `scripts/smoke_test.py`, on any branch — while `CLAUDE.md` documented both as the mandatory pre-flight gate. The documented final verification step was fictional. | Built for real: `tests/test_smoke.py` boots seeding → StubMarket heartbeat → LLM/compiled/benchmark decision cycles → risk gate → paper fill → SL-wick close with shared-cost fees+funding → wait-candidate → counterfactual replay → state snapshot (~0.7s). `scripts/smoke_test.py` is the runner. CLAUDE.md corrected, with a note that "documented" ≠ verified. Its first run immediately caught F13. |
| F6 | High | **Reflection armed with no log row ever written.** Settings table was empty → trigger defaulted to `trade_count/20` (not manual), and nothing in production INSERTed into `reflections` (`run_reflection_cycle` only UPDATEd rows that don't exist) — so any agent past 20 closed trades re-fired a full reflection (two LLM calls) every 30 minutes forever. The R12 zero-evidence deploy guard held, but the loop was a self-inflicted LLM DoS. | `run_reflection_cycle` now INSERTs the reflections row at trigger time and UPDATEs it by rowid (outcome `deployed`/`rejected`/`not_triggered`) — eligibility pacing is real in every path. Additionally R12 Latch 2 route (a) applied: settings `reflection_trigger = {"mode": "manual"}` for the run. |
| F7 | High | **btc_hold stacked positions.** No once-only guard: it re-entered every cycle (3 stacked BTC longs in 17 minutes in the live DB) until the 3-position cap spammed `risk_blocked`; the bridge's 48h default max_hold would then churn it (close, re-enter, fees) every two days. The null benchmark measured nothing like buy-and-hold (R2 AC#4 unmet). | `decision_loop` benchmark path: waits (`"benchmark holding BTC"`) when a BTC position exists; enter carries `max_hold_hours = 8760`. Test: `test_btc_hold_enters_once_and_holds` + smoke assertion. |
| F8 | High | **C7 junk spawner still scheduled hourly.** `meta/head_of_desk.py` unchanged (top-level `target_agent_count` default 5, hardcoded archetypes, graveyard bypassed). Dormant at 9 active agents, but any suspensions (accelerated by F1's inflated drawdowns) dropping the count below 5 would mint `agent_momentum_1`-style agents mid-run. | Job registration removed from `forge.py` with an R12-style latch note pointing at R7 (the module and its tests remain for R7 to rebuild). Startup log states the job is disabled. |
| F9 | Medium | **Three sizing states.** Live scaled `size × confidence` *after* the risk gate validated the unscaled order; backtest used `0.5 + 0.5·scale` between spec thresholds. R4 AC#4 ("no third state; gate validates the final order") unmet. | One formula in new `execution/sizing.py` (`scale_position_size`; thesis rule: full size ≥ 0.70, linear half-to-full between 0.50–0.70, spec thresholds override via the compiled response). Live applies it *before* `validate_order`; `backtest/engine.py` calls the same helper. `tests/test_sizing.py` pins formula + engine parity. |
| F10 | Low | Latent `NameError`: `store/positions.py execute_close` no-account fallback referenced undefined `notional`. | Uses the already-computed `margin`. |
| F11 | Low | **Working-tree pollution.** `meta/spawner.py` writes thesis files unconditionally, so test runs kept re-creating junk (`test_trader_v1.md`, `agent_momentum_1_v1.md`, …) that R11 had deleted; benchmark theses sat untracked; `.opencode/` debris untracked. | Autouse conftest fixture redirects `meta.spawner._THESES_DIR` (and the smoke env redirects `scripts.seed_benchmarks._THESES_DIR`, spec dirs, OI-history path) to tmp. Junk deleted again; benchmark theses committed; `.opencode/` gitignored. |
| F12 | **Blocker** (found during fix work) | **Counterfactual ledger replay was dead code in production.** The real ledger writes candle `ts` as ISO strings; `store/counterfactuals._read_candle_ledger` normalised with `pd.to_numeric` → every ISO row became NaN → the ledger path silently returned nothing and replay fell through to the 25h heartbeat window. Waits older than ~25h could never be filled — even with F4 fixed. | ISO-or-numeric ts normalisation to epoch-ms (and the returned rows carry numeric ts for `find_first_cross`). Test writes a real-format ISO partition and asserts the wait fills. Found because the smoke-harness work checked the actual partition format instead of trusting the synthetic numeric-ts fixtures. |
| F13 | Low (found by F5's harness, first run) | Compiled agents that *wait* never updated `last_model_used` (only the enter path did), breaking the "every cycle records its model" contract for exactly the agents that wait most. | `update_last_model_used` added to all three compiled wait branches. |

### 9.2 R12 operating notes (which latch mode is active)

Recorded per R12 AC#4 so R6/R7/R8 know exactly what they are replacing:

- **Latch 1 (meta-controller):** code-guard route — `meta/evaluator.py` `null_valid` requires `benchmark_random_walk` metrics with ≥30 closed trades before any not-beating-null suspend/terminate. Verified present.
- **Latch 2 (reflection):** *both* routes — code-guard (zero-evidence deploy rejection in `agents/reflection.py`) *and* settings `reflection_trigger = {"mode": "manual"}` (set 2026-07-11 after re-seed). R8 should flip the setting back to `trade_count` only after its real gates land; the F6 log-row fix stays either way.
- **Latch 3 (head of desk, added by this review):** job-disable route — registration removed from `forge.py`; remove the latch only with R7's rebuilt spawn path.

### 9.3 Launch checklist (run this before starting the two-week run)

1. `C:\ProgramData\Anaconda3\python.exe -m pytest tests/ --ignore=tests/test_forge_agent_timeout.py --ignore=tests/test_forge_heartbeat_schedule.py` → all green (2026-07-11: **579 passed, 1 skipped**).
2. `python scripts/smoke_test.py` → passes (2026-07-11: passes, ~0.7s).
3. Fresh DB: `python scripts/fresh_start.py --yes` — then verify `config_json` shows 4× `compiled: true`, 3× `pinned_model`, and 4 active `specs` rows (done 2026-07-11).
4. Benchmarks seeded (`scripts/seed_benchmarks.py` or first forge boot) and `reflection_trigger` = manual in settings (done 2026-07-11).
5. Boot check: `python forge.py` startup log must show both R12 latch lines, "Head of desk job DISABLED", and heartbeat/counterfactual/git-sync/meta-controller/reflection/risk-officer schedulers registered.
6. During the run: do **not** touch Promote-to-Shadow / Go-Live (they silently stop an agent — C9, unfixed until R9) and avoid Emergency Stop (graveyard corruption — C9). The run branch is `feat/r5-cost-model`; `main` is behind — merge or accept running from the branch, but do not mix.

**Process lesson, repeated.** F1/F2/F5 are the same failure §6.7 named: tests written to mirror code, and documentation written ahead of reality. The smoke harness caught F13 on its first execution and F12 fell out of checking real data formats — the two cheapest catches in this whole exercise. "Wiring tests or it didn't happen" now includes: *expected values in tests come from the domain (an exchange statement, a hand calculation), never from running the code under test.*

*Appended by the desk's reviewing agent, 2026-07-11, after the pre-launch re-audit and same-day fixes on `feat/r5-cost-model`.*

### 9.4 Mid-run operational log (the two-week run)

Changes deployed into the live desk mid-sample, per the §8 rule ("only what the run itself breaks"):

- **2026-07-12 — `.sh` popup storm (environment, not Forge code).** Every LLM decision call spawned a blocking Windows "How do you want to open this file?" dialog. Root cause chain: the oh-my-openagent opencode plugin replays Claude Code hooks inside each `opencode run` session that `model_chain` spawns; the user-level Claude Stop hook was a bare `.sh` path; and on this Windows 11 machine, ShellExecuting any `.sh` always opens the picker (verified empirically — a valid HKLM association is ignored without a UserChoice). Fix outside the repo: `~/.claude/settings.json` Stop hook now invokes Git bash explicitly (backup `settings.json.bak-2026-07-12`). Sharp edge documented in `CLAUDE.md`. Verified clean across 2+ fleet cycles. No desk restart.
- **2026-07-13 — control arm dead on arrival, re-pinned to the local model (owner decision).** The `openrouter/anthropic/claude-sonnet-5` pin failed on 100% of cycles from launch (84/84 per agent over ~7h): OpenRouter HTTP 402, the key has ~$0.30 of credit and opencode requests 32k max output tokens. Owner chose re-pinning to the local Qwen over funding. Landed in two steps:
  1. *First attempt — forge-managed llama-server: rejected after measurement.* `_tier_from_pinned_value` gained a `"llama_server"` pin; settings pointed at Ollama's bundled `llama-server.exe` + the `Qwen3.6-35B-A3B-UD-Q4_K_S.gguf`; server started in-process via `POST /api/local-server/start`. A trivial smoke prompt answered in 8s — but real ~10k-token prompts never returned: the standalone binary loads no CUDA backend (its GPU dlls live in `cuda_v12/` subdirs it never scans), so the 35B ran CPU-only (RTX 3080 at 2%, 39 GB RSS, >10-minute prefill) and every fleet request died at the client's 60s timeout, all three agents erroring in lockstep. Server stopped; `spawn_on_startup` back to false. Lesson repeated from §9: a smoke test at toy scale proves wiring, not capacity — measure at real input size.
  2. *Final — Ollama service, thinking off.* `llm/ollama_client.py` now sends `"think": false` (overridable via `config["llm_think"]`): measured 13.2s warm on an 18.5k-token prompt (vs 121-194s with thinking), clean JSON. `_tier_from_pinned_value` maps the pin literal `"ollama"` to the Ollama tier; control arm re-pinned to `"ollama"` in the live DB and `scripts/fresh_start.py`. Verified end-to-end through `model_chain.decide` as copper_vane: 2.9s warm, model label "Local Ollama (qwen3.6:35b_optimized, thinking off)". No desk restart at any point (agent runners are fresh processes each cycle).

  **Analysis note:** the control arm's sample effectively begins 2026-07-13 ~05:00 UTC; its first ~8h are error rows (zero trades, zero P&L impact). Compiled-vs-control comparisons should date the control arm from the re-pin, and the arm now answers "does LLM reasoning (local Qwen, thinking off) beat compiled specs?" — not the original frontier-model question.
